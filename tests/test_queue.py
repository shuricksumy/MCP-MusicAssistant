...
async def test_queue_get_fetches_state_and_items(patched_client):
    ...
    result = await queue(player="DX3 Pro", action="get")
...
async def test_queue_shuffle_requires_bool(patched_client):
    ...
    with pytest.raises(ValueError):
        await queue(player="DX3 Pro", action="shuffle")


async def test_queue_shuffle_sends_command(patched_client):
    patched_client()
    result = await queue(player="DX3 Pro", action="shuffle", shuffle=True)
    assert result == {"player": "DX3 Pro", "shuffle": True}


async def test_queue_repeat_validates_mode(patched_client):
    patched_client()
    with pytest.raises(ValueError):
        await queue(player="DX3 Pro", action="repeat", repeat="loop-forever")


async def test_queue_move_requires_item_id(patched_client):
    patched_client()
    with pytest.raises(ValueError):
        await queue(player="DX3 Pro", action="move_up")


async def test_queue_move_up_sends_negative_shift(patched_client):
    patched_client()
    result = await queue(player="DX3 Pro", action="move_up", item_id="item-5")
    assert result == {"player": "DX3 Pro", "action": "move_up", "item_id": "item-5"}


async def test_queue_unknown_action_raises(patched_client):
    patched_client()
    with pytest.raises(ValueError):
        await queue(player="DX3 Pro", action="teleport")


async def test_queue_move_next_sends_zero_shift(patched_client):
    client = patched_client()
    await queue(player="DX3 Pro", action="move_next", item_id="item-5")
...
async def test_queue_remove_uses_item_id_or_index_kwarg(patched_client):
    client = patched_client()
    await queue(player="DX3 Pro", action="remove", item_id="item-5")
    command, kwargs = client.calls[-1]
